...
#include <sys/types.h>
#include <jni.h>
#include "../../../../../../Library/Android/sdk/ndk-bundle/sysroot/usr/include/stdio.h"
#include "../jni/bzip2/bzlib.h"
...
int patchMethod(int argc,char * argv[])
{
    ...
    */
    ...
    if((newReplace= static_cast<u_char *>(malloc(newsize + 1))) == NULL) err(1, NULL);
    ...
    while(newpos<newsize) {
        ...
        /* Read diff string */
        lenread = BZ2_bzRead(&dbz2err, dpfbz2, newReplace + newpos, ctrl[0]);
        if ((lenread < ctrl[0]) ||
        ...
        for(i=0;i<ctrl[0];i++)
            if((oldpos+i>=0) && (oldpos+i<oldsize))
                newReplace[newpos+i]+=old[oldpos+i];
        ...
        /* Read extra string */
        lenread = BZ2_bzRead(&ebz2err, epfbz2, newReplace + newpos, ctrl[1]);
        if ((lenread < ctrl[1]) ||
    ...
    if(((fd=open(argv[2],O_CREAT|O_TRUNC|O_WRONLY,0666))<0) ||
       (write(fd,newReplace,newsize)!=newsize) || (close(fd)==-1))
        err(1,"%s",argv[2]);
...
Java_com_example_addapkupgradedemo_MainActivity_bspatch(JNIEnv *env, jclass type, jstring oldApk_,
